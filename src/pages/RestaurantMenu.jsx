...
const RestaurantMenu = () => {
  ...
  const handleIncrement = (category, itemName) => {
    setFoodItems((prevFoodItems) => {
      ...
      if (!updatedFoodItems[category][itemName].count) {
        // If count is not present, initialize it to 1
  ...
  const scrollToCategory = (category) => {
    if (categoryRefs[category]) {
      categoryRefs[category].scrollIntoView({ behavior: 'smooth', block: 'center' });
    }
  };
    ...
    <div className="container w-[55rem] mx-auto p-4 mt-28">
      <div className='flex h-fit bg-gradient-to-r from-amber-200 to-orange-400/90 py-8 px-4 rounded-md shadow-md'>
        <div>
          ...
          <p className='text-gray-500 font-semibold text-sm mt-1 ml-1 '>{restaurant.cuisine}</p>
          <p className='text-gray-500 font-semibold text-lg mt-4 ml-1 '>{restaurant.address}</p>
      ...
      <div className="flex my-4">
        <button
          className={`mr-4 px-4 py-2 text-xl font-semibold ${
            activeTab === 'menu' ? 'text-amber-500 border-b-2 border-amber-500' : 'text-gray-400'
          } `}
        ...
        <button
          className={`px-4 py-2 text-xl font-semibold ${
            activeTab === 'reviews' ? ' text-amber-500 border-b-2 border-amber-500' : 'text-gray-400'
          } `}
